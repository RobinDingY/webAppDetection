...
Original file is located at
    https://colab.research.google.com/drive/1QxcfWbrMcf1OhivNrfCiDg7wTnlERgF7
"""
...
class Hand_Face_Tracker():
    ...
    Args:
        detect_model: path to the palm_detection.tflite or face model
        joint_model: path to the hand_landmark.tflite(dace not have)
    ...
    Examples::
        ...
        >>> input_img = np.random.randint(0,255, 256*256*3).reshape(256,256,3)/input_img = np.random.randint(0,255, 128*128*3).reshape(128,128,3)
        >>> keypoints, bbox = det(input_img)
    """

    def __init__(self,detect_type, detect_model, joint_model, anchors_path,
                 box_enlarge=1.5, box_shift=0.2):
        self.dtype = detect_type
        self.record_mark = [False]*2944
        # self.d_record = (None,None)
        ...
        if self.dtype == 'hand':
            ...
            self.interp_palm = tf.lite.Interpreter(detect_model)
        ...
        elif self.dtype == 'face':
            self.interp_face = tf.lite.Interpreter(detect_model)
    ...
    def non_maximum_suppression(self, reg, anchors, scores, weighted=True, sim_thresh=0.3, max_results=-1):
        '''
        ...
        while len(remain_idxs) > 0:
            ...
            if max_results > 0 and output_regs.shape[0] >= max_results:
                break
        return output_regs
    # def similarity(self,new):
    #     s = cosine_similarity([self.record_mark], [new])
    #     if s >=0.2:
    #         return self.record_mark 
    #     else:
    #         return new
    def detect(self, img_norm):
        if self.dtype == 'hand':
            ...
            # print(out_reg.shape,out_clf.shape,out_scr.shape)#(2944, 18) (2944,) (2944,)

            detection_mask = out_scr > 0.7
            #将detection_mask记录,如果下一张的图片的detection_mark和这个差不多(0.9),直接返回上一张图片的数据
            # if cosine_similarity([self.record_mark], [detection_mask]) <= 0.9:
            self.record_mark = detection_mask
            filtered_detect = out_reg[self.record_mark]#将大于0.7的行保留下来:(x,18)
            filtered_anchors = self.anchors[self.record_mark]#(x,4)
            filtered_scores = out_scr[self.record_mark]      
            ...
            candidate_detect = self.non_maximum_suppression(filtered_detect, filtered_anchors, filtered_scores)#(1,18)

            # bounding box center offsets, width and height
            dx, dy, w, h = candidate_detect[0, :4]# == [:4]
            # 7 initial keypoints
            keypoints = candidate_detect[0,4:].reshape(-1,2) #(7,2)         
            ...
            # end = time.time()
            # print(end-start) 
            # self.d_record = (source,keypoints)
    ...
    def __call__(self, img):
        ...
        if self.dtype == 'hand':
            source, keypoints = self.detect(img_norm)
            if source is None:
                return None, None
            ...
            bboxs, keyps = self.detect(img_norm)
            if bboxs is None:
                return None, None
...
class MultiHandTracker3D():
    ...
    def _GetCandidateIdx(self, box_list, max_hands = 1, iou_thres = 0.45):
